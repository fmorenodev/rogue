...
func add_type(syringe_type: int) -> void:
	...
	match type:
		en.SYRINGE_TYPE.HEALTH_S:
			texture = hs_texture
			entity_name = tr("SYRINGE").format([tr("SIZE_SMALL").capitalize(), tr("EFFECT_HEALTH")])
			desc = tr("SYRINGE_HS_DESC")
		en.SYRINGE_TYPE.HEALTH_L:
			texture = hl_texture
			entity_name = tr("SYRINGE").format([tr("SIZE_LARGE").capitalize(), tr("EFFECT_HEALTH")])
			desc = tr("SYRINGE_HL_DESC")
		en.SYRINGE_TYPE.DEFENSE_S:
			texture = ds_texture
			entity_name = tr("SYRINGE").format([tr("SIZE_SMALL").capitalize(), tr("EFFECT_DEFENSE")])
			desc = tr("SYRINGE_DS_DESC")
		en.SYRINGE_TYPE.DEFENSE_L, _:
			texture = dl_texture
			entity_name = tr("SYRINGE").format([tr("SIZE_LARGE").capitalize(), tr("EFFECT_DEFENSE")])
...
func use(user: Node2D, item_index: int) -> void:
	match type:
		...
		en.SYRINGE_TYPE.DEFENSE_L, _:
			...
			# timer
	events.emit_signal("new_message", tr("ITEM_USE"), color.white, [entity_name])
